...
class ActiveRecyclerViewAdapter(val context: Context, var list: ArrayList<ICO>) : RecyclerView.Adapter<ActiveRecyclerViewAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if (target.fix.toInt() == 1) {
            holder.rlInnerBackground.setBackgroundResource(R.color.status_now)
            holder.tvPrimary.setTextColor(ContextCompat.getColor(context, android.R.color.white))
            holder.tvSecondary.setTextColor(ContextCompat.getColor(context, android.R.color.white))
            holder.tvTitle.setTextColor(ContextCompat.getColor(context, android.R.color.white))
            holder.tvDescript.setTextColor(ContextCompat.getColor(context, android.R.color.white))
            holder.vSeparator.setBackgroundColor(ContextCompat.getColor(context, android.R.color.white))
        } else {
            holder.rlInnerBackground.setBackgroundResource(R.color.background)
            holder.tvPrimary.setTextColor(ContextCompat.getColor(context, android.R.color.black))
            holder.tvSecondary.setTextColor(ContextCompat.getColor(context, android.R.color.black))
            holder.tvTitle.setTextColor(ContextCompat.getColor(context, android.R.color.black))
            holder.tvDescript.setTextColor(ContextCompat.getColor(context, android.R.color.black))
...
//                    && System.currentTimeMillis() < millis.toLong()
    ...
    fun isNew(millis: String): Boolean {
       return millis.toLong() > 0 && TimeUnit.HOURS.convert(System.currentTimeMillis() - millis.toLong(), TimeUnit.MILLISECONDS).toInt() < 72
    }
    ...
    inner class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView), View.OnClickListener, View.OnLongClickListener {
        ...
        var rlInnerBackground: RelativeLayout
        ...
        init {
            ...
            rlInnerBackground = itemView.findViewById<RelativeLayout>(R.id.rlInnerBackground)
            rlBackground = itemView.findViewById<RelativeLayout>(R.id.rlBackground)
            vSeparator = itemView.findViewById<View>(R.id.vSeparator)
