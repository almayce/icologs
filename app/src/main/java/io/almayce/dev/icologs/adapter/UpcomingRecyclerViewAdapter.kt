...
class UpcomingRecyclerViewAdapter(val context: Context, var list: ArrayList<ICO>) : RecyclerView.Adapter<UpcomingRecyclerViewAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if (target.fix.toInt() == 1) {
            holder.rlInnerBackground.setBackgroundResource(R.color.status_up)
...
//        holder.rlPreSale.layoutParams.width = if (isPreSale(target.preSaleDate)) ViewGroup.LayoutParams.WRAP_CONTENT else 0
        ...
        println(TimeUnit.HOURS.convert(System.currentTimeMillis() - target.newDate.toLong(), TimeUnit.MILLISECONDS).toInt())
